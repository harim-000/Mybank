<template>
  <v-card  max-width="3000">
    ...
    <div v-if="myQuestionList === null || myQuestionList.length === 0">
      <br>
      <br>
      <br>
      <br>
      <h3 align="center" style="color : #7E57C2">내가 작성한 질문글이 없습니다.</h3>
      <br>
      <br>
      <br>
      <br>
    ...
    <div v-else>
      ...
      >
        ...
        <tr v-for="(item, index) in myQuestionList" :key="index">
          <td style="text-align: left">
            <v-btn
                variant="text"
                v-if="item.user_id === curr_user_id || item.open === '0'"
...
export default {
  data() {
    return {
      ...
      myQuestionList: [] // 나의 질문글 목록을 저장할 배열
    };
  },
  watch: {
    page: {
      handler: 'fetchMyQuestionList',
  ...
  methods: {
    ...
    async fetchMyQuestionList() {
      ...
      try {
        const response = await axios.post(`/faq/my-questions?user_id=${this.curr_user_id}&page=${this.page}&itemsPerPage=${this.itemsPerPage}`);
        console.log(response.data)
        this.totalItems = response.data.totalItems;
        this.totalPages = response.data.totalPages;
        this.myQuestionList = response.data.faqList; // 서버로부터 받아온 나의 질문글 목록을 설정
        console.log(this.myQuestionList)
      } catch (error) {
        console.error('나의 질문글 목록 불러오기 오류:', error);
      }
    },
    ...
    async onPageChange(newPage) {
      this.page = newPage;
      console.log(newPage);
      await this.fetchMyQuestionList();
    },
  ...
  created() {
    this.fetchMyQuestionList(); // 컴포넌트 생성 시에 나의 질문글 목록을 불러오도록 호출
  }
};
</script>
